    ...
    public class KafkaResponseHandler : IDisposable
    {
        ...
        public KafkaResponseHandler(IConfiguration configuration)
        {
            ...
            var consumerConfig = new ConsumerConfig
            {
                ...
                GroupId = $"order-service-response-group-{Guid.NewGuid()}", // Уникальный GroupId
                AutoOffsetReset = AutoOffsetReset.Latest // Начинаем с последних сообщений
            };
        ...
        {
            Log.Information("⚙️ KafkaResponseHandler начал слушать ответы...");
            while (_isRunning)
            {
                try
                {
                    ...
                    {
                        Log.Information("⏭️ Ответ пропущен: неподходящий тип события");
                        continue;
                    }
                    ...
                    {
                        Log.Warning($"⚠️ Не удалось распарсить время сообщения: {timestamp}");
                    }
